...
from posteriors.be.batchensemble_layers import *
...
class Wide_ResNet_BatchEnsemble(nn.Module):
    def __init__(self, channels, depth, widen_factor, dropout_rate, num_classes, num_models, pool_number=8):
        ...
        self.conv1 = conv3x3(channels,nStages[0], stride=1,first_layer=True, num_models=num_models)
    ...
    def forward(self, x):
        ...
        # print(out.shape)
        out = F.avg_pool2d(out, self.pool_number)
        # print(out.shape)
        ...
        if not self.training:
            ...
            if self.variance:
                return out.view([self.num_models, -1, self.num_classes])
            else:
                return out.view([self.num_models, -1, self.num_classes]).mean(0)
