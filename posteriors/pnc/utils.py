...
def train(args, model, device, train_loader, optimizer, epoch, verbose):
    model.train()
...
def generate_psuedo_labels(args, data, T_POINT, device, test_feature = None, N = 5000, width = 1):
    ...
    for i in range(N):
        model = Net(input_channel = data.shape[-1], hidden_layer=args.data_length*width).to(device)
