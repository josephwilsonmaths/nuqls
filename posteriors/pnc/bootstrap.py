...
def bootstrap(args, T_POINT, T_y, N_boot, device, net_width=32, verbose=False):
    ...
    B_pesudo, averaged_T = generate_psuedo_labels(args, X_train[B_set_index], T_POINT, device,
                                                  test_feature = None, N = 5000, width=net_width)

    X_Bset, Y_Bset = X_train[B_set_index], B_pesudo

    # get the initial network
    model0 = Net(input_channel = X_train.shape[-1], hidden_layer=args.data_length*net_width).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model0, device, A_loader_base, optimizer, epoch, verbose)
    if args.save_model:
    ...
    B_loader_base = torch.utils.data.DataLoader(dataset=B_data_base,
                                               batch_size=args.batch_size,
                                               shuffle=True)
    model_new = Net(input_channel = X_train.shape[-1], hidden_layer=args.data_length*net_width).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model_new, device, B_loader_base, optimizer_new, epoch, verbose)
    ...
    for i in range(N_boot):
        ...
        for epoch in range(1, args.epochs + 1):
            train(args, model0, device, A_loader_sub, optimizer, epoch, verbose)
        if args.save_model:
            PATH_final = 'model0_final.pth'
            torch.save(model0.state_dict(), PATH_final)
        ...
        optimizer_new = optim.SGD(model_new.parameters(), lr=args.lr, weight_decay=args.wd)
        for epoch in range(1, args.epochs + 1):
            train(args, model_new, device, B_loader_sub, optimizer_new, epoch, verbose)
