...
from torch.func import vmap, jacrev, grad, jvp, vjp
import numpy as np
...
class linear_ntk_c(object):
    ...
    def method(self,test,ood_test=None,mu=0,weight_decay=0,verbose=False, progress_bar = True, gradnorm=False, netbase=None,in_channels=1,num_classes=10):
        ...
        if netbase is None:
            ...
            self.theta = torch.empty(size=(self.theta_t.shape[0],self.S),device=device)
            for s in range(self.S):
                if netbase == 'lenet':
                    he_net = df2.LeNet5().to(device)
                elif netbase == 'resnet':
                    he_net = model.ResNet9(in_channels=in_channels, num_classes=num_classes).to(device)
                he_net.apply(init_weights_he)
                he_net.eval()

                self.theta[:,s] = nn.utils.parameters_to_vector(he_net.parameters()).detach() + self.theta_t
